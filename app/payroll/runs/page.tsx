...
export default function PayrollRunsPage() {
    ...
    const [formData, setFormData] = React.useState<{
        ...
        start_date: string;
        end_date: string;
        group_id: string; // Changed to string for RadioGroup value
    }>({ cycle_name: '', start_date: '', end_date: '', group_id: '' });
    ...
    const resetForm = () => {
        setFormData({ cycle_name: '', start_date: '', end_date: '', group_id: '' });
    };

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.group_id) {
            toast({ title: "Validation Error", description: "You must select a payroll group.", variant: "destructive"});
            return;
        }
        setIsSubmitting(true);
        try {
            // Convert group_id back to number and wrap in array for API
            await createPayrollCycle({
                ...formData,
                group_ids: [Number(formData.group_id)]
            });
            toast({ title: "Success", description: "Payroll cycle created successfully." });
            fetchData();
            setIsCreateDialogOpen(false);
             ...
             <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                 <DialogContent className="sm:max-w-lg">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 py-4">
                        ...
                        <div className="grid gap-2">
                            <Label>Select Payroll Group</Label>
                            {isLoading ? (
                                <ScrollArea className="h-32 rounded-md border p-4">
                                    <div className="space-y-2">
                                        {Array.from({ length: 3 }).map((_, idx) => (
