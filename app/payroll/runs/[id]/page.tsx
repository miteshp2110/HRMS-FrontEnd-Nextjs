...
import { PayrollDownloadButton } from "@/components/ui/downloadButton";
...
export default function PayrollDetailPage({ params }: PayrollDetailPageProps) {
  ...
  const [editingDetail, setEditingDetail] = useState<PayslipDetail | null>(
    null
  );
  ...
  const loadPayrollData = async () => {
    ...
    try {
      ...
      const currentRun = runs.find(
        (run) => run.id === Number.parseInt(payrollId)
      );
      ...
      toast({
        title: "Error",
        description: `Failed to load payroll data: ${error.message}`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const loadPayslipDetails = async (payslipId: number) => {
    try {
      ...
      toast({
        title: "Error",
        description: `Failed to load payslip details: ${error.message}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDetail) {
        ...
        toast({
          title: "Success",
          description: "Payslip component updated successfully",
        });
      } else {
        await addPayslipDetail(selectedPayslip.id, formData);
        toast({
          title: "Success",
          description: "Payslip component added successfully",
        });
      }
      setDialogOpen(false);
  ...
  const handleDelete = async (detailId: number) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Payslip component deleted successfully",
      });
      loadPayslipDetails(selectedPayslip.id);
      loadPayrollData();
    } catch (error: any) {
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4 p-4">
          ...
          <Button variant="outline" asChild>
            <Link href="/payroll/runs" className="flex items-center">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {`${new Date(payrollRun.pay_period_start)
                .toLocaleDateString("en-GB")
                .replace(/\//g, "-")} to ${new Date(payrollRun.pay_period_end)
                .toLocaleDateString("en-GB")
                .replace(/\//g, "-")}`}
            </p>
          </div>
          ...
          <div className="lg:col-span-1">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {filteredAndSortedPayslips.length} of {payslips.length}{" "}
                  employees
                  {filteredAndSortedPayslips.some((p) =>
                    isNegativeOrZeroAmount(p.net_pay)
                  ) && (
              ...
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    >
                      {selectedPayslip
                        ? `${selectedPayslip.employee_name}'s Payslip`
                        : "Select an Employee"}
                    </CardTitle>
                    <CardDescription>
                      {selectedPayslip ? (
                  ...
                  {selectedPayslip && payrollRun.status === "processing" && (
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {editingDetail ? "Edit Component" : "Add New Component"}
              </DialogTitle>
              <DialogDescription>
                {editingDetail
