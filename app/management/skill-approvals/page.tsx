...
import { Award, Search, CheckCircle, XCircle, Clock, AlertCircle, UserCheck } from "lucide-react"
import { getPendingSkillApprovals, approveSkillRequest, getSkills, getEmployeesBySkill, type SkillApproval, type Skill, type SkilledEmployee } from "@/lib/api"
import { useToast } from "@/hooks/use-toast"
...
const SkillMatrixView = () => {
    ...
    useEffect(() => {
        getSkills().then(setAllSkills).catch(err => {
            toast({ title: "Error", description: "Could not load the skills library.", variant: "destructive"})
        })
    }, [toast]);
    ...
    useEffect(() => {
        if (selectedSkill) {
            ...
            getEmployeesBySkill(selectedSkill)
                ...
                .catch(err => toast({ title: "Error", description: `Could not load employees for ${selectedSkill}.`, variant: "destructive" }))
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Select a skill to see which employees have been approved for it.</CardDescription>
                <div className="pt-4">
            ...
            <CardContent>
                {isLoading ? <div className="text-center py-8">Loading skilled employees...</div> :
                !selectedSkill ? <div className="text-center py-12 text-muted-foreground"><UserCheck className="h-12 w-12 mx-auto mb-4 text-gray-400"/>Please select a skill to begin.</div> :
                skilledEmployees.length === 0 ? <div className="text-center py-12 text-muted-foreground"><UserCheck className="h-12 w-12 mx-auto mb-4 text-gray-400"/>No employees have this skill verified yet.</div> :
                (
                        ...
                        {skilledEmployees.map(emp => (
                            <Card key={emp.employee_id}>
                                <CardContent className="pt-6 flex flex-col items-center text-center">
                                    ...
                                    <p className="text-xs text-muted-foreground">Approved by {emp.approved_by_name || 'N/A'}</p>
                                    <Button variant="ghost" size="sm" className="mt-4" asChild>
                                        <Link href={`/directory/${emp.employee_id}`}>View Profile</Link>
                                    </Button>
                                </CardContent>
                            </Card>
                        ))}
...
export default function SkillApprovalsPage() {
  ...
  const fetchApprovals = async () => {
    if (!canManageSkills) { setLoading(false); return; }
    try {
      setLoading(true);
      const data = await getPendingSkillApprovals()
      setApprovals(data as SkillApproval[]|[]);
                    ...
                    <TabsContent value="approvals" className="mt-6">
                        <Card>
                            <CardHeader>
                                ...
                                <div className="relative pt-4">
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                                    <Input placeholder="Search by employee name or skill..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/>
                                </div>
                            </CardHeader>
                            <CardContent>
                            {loading ? ( <div className="text-center py-8">Loading pending approvals...</div> ) : 
                             filteredApprovals.length === 0 ? (
                                <div className="text-center py-12 text-muted-foreground">
                                    <CheckCircle className="h-10 w-10 mx-auto mb-4 text-green-500"/>
                                    <p>No pending skill approvals found. All caught up!</p>
                                ...
                                <Table>
                                <TableHeader><TableRow><TableHead>Employee</TableHead><TableHead>Skill Requested</TableHead><TableHead>Requested On</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                                <TableBody>
                                    {filteredApprovals.map((approval) => (
                                    <TableRow key={approval.id} onClick={() => handleRowClick(approval)} className="cursor-pointer">
                                        <TableCell className="font-medium">{approval.first_name} {approval.last_name}</TableCell>
                                        <TableCell>{approval.skill_name}</TableCell>
                                        <TableCell>{new Date(approval.created_at).toLocaleDateString()}</TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex gap-2 justify-end" onClick={(e) => e.stopPropagation()}>
                                                <Button size="sm" variant="outline" className="border-green-500 text-green-600 hover:bg-green-50" onClick={() => handleApprovalUpdate(approval.id, 1)}><CheckCircle className="h-4 w-4 mr-2" /> Approve</Button>
                                                <Button size="sm" variant="outline" className="border-red-500 text-red-600 hover:bg-red-50" onClick={() => handleApprovalUpdate(approval.id, 0)}><XCircle className="h-4 w-4 mr-2" /> Reject</Button>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                    ))}
