...
function SkillApprovalSkeleton() {
  return (
...
export function PendingSkillApprovalWidget() {
  ...
  useEffect(() => {
    setIsLoading(true);
    getPendingSkillApprovalsDashboard()
      .then(setSkills)
      .catch(console.error)
      .finally(() => setIsLoading(false));
  }, []);

  if (isLoading) return <SkillApprovalSkeleton />;
  ...
  return (
    <div className="rounded-xl p-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow duration-300 max-h-[400px] h-[400px] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        ...
        {/* Count badge */}
        <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-900/50">
          <Award className="h-3 w-3 text-indigo-600 dark:text-indigo-500" />
          ...
          >
            <div className="flex items-center gap-2.5 flex-1 min-w-0">
              <Avatar className="h-9 w-9 border-2 border-white dark:border-zinc-800 shadow-sm flex-shrink-0">
                ...
                <AvatarFallback className="bg-gradient-to-br from-indigo-500 to-indigo-600 text-white text-xs font-medium">
                  {skill.employee_name.charAt(0)}
                </AvatarFallback>
              </Avatar>
            ...
            >
              <Link href="/management/skill-approvals" className="flex items-center gap-1">
                Review
