...
import { HandCoins, ArrowRight, Send } from "lucide-react";

function LoanDisbursementSkeleton() {
  return (
    <div className="rounded-xl p-6 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-sm max-h-[400px] h-[400px] flex flex-col">
      <div className="flex items-center justify-between mb-5 animate-pulse">
        <div className="space-y-2">
          ...
          <div className="h-4 w-52 rounded bg-slate-100 dark:bg-zinc-800/60" />
        </div>
      ...
      <div className="flex-1 space-y-3 overflow-hidden">
        {[...Array(4)].map((_, idx) => (
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div className="h-7 w-28 bg-slate-100 dark:bg-zinc-800 rounded" />
...
export function LoanDisbursementWidget() {
  ...
  if (isLoading) return <LoanDisbursementSkeleton />;

  // Calculate total loan disbursement amount
  const totalAmount = requests.reduce((sum, req) => sum + Number(req.approved_amount), 0);
      ...
      <div className="rounded-xl p-6 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-sm max-h-[400px] h-[400px] flex flex-col">
        <div className="mb-5">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Approved loans ready for payment
          </p>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-500">
              All loan disbursements have been processed
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="rounded-xl p-6 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow duration-300 max-h-[400px] h-[400px] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-5 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-900/50">
            <HandCoins className="h-3.5 w-3.5 text-purple-600 dark:text-purple-500" />
            <span className="text-sm font-semibold text-purple-700 dark:text-purple-400">
              {requests.length}
            </span>
          </div>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <div className="text-right">
                <p className="text-sm font-semibold text-purple-700 dark:text-purple-400">
                  AED {req.approved_amount.toLocaleString()}
                </p>
              </div>
