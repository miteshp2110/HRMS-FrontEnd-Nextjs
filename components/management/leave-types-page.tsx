...
import { Plus, Edit, Trash2, AlertCircle, Check, X, Clock, Calendar } from "lucide-react"
...
export function LeaveTypesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const dataToSubmit = {
      ...formData,
      accural_rate: formData.accurable ? formData.accural_rate : 0,
    }
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Get started by creating your first leave type for employees to use.
              </p>
              <Button onClick={() => handleOpenDialog()}>
                <Plus className="h-4 w-4 mr-2" />
                Create Leave Type
              ...
              <TableBody>
                {leaveTypes.map((type) => (
                  <TableRow key={type.id}>
                    ...
                    <TableCell>
                      {type.is_encashable ? <Check className="h-5 w-5 text-green-500" /> : <X className="h-5 w-5 text-muted-foreground" />}
                    </TableCell>
