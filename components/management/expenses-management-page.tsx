...
import { Plus, Receipt, AlertCircle, Eye, Loader2 } from "lucide-react"
import { getExpenseApprovals, getExpenseClaims, type ExpenseClaim } from "@/lib/api"
import { useToast } from "@/hooks/use-toast"
import { AdminExpenseDetailsDialog } from "./expense-details-dialog"
import { AdvanceDialog } from "./advance-dialog"
...
export function ExpensesManagementPage() {
  ...
  const [isDetailsDialogOpen, setIsDetailsDialogOpen] = React.useState(false)
  const [isAdvanceDialogOpen, setIsAdvanceDialogOpen] = React.useState(false)
  const [openingClaimId, setOpeningClaimId] = React.useState<string | number | null>(null)
  ...
  const fetchData = React.useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const [approvals, all] = await Promise.all([getExpenseApprovals(), getExpenseClaims()])
      setPendingApprovals(approvals)
      setAllClaims(all)
    } catch (error: any) {
      toast({
  ...
  const handleReviewClick = (claim: ExpenseClaim) => {
    setOpeningClaimId(claim.id)
    setActiveClaim(claim)
    setIsDetailsDialogOpen(true)
    // clear spinner immediately on open; if your dialog fetches more data, you can clear after it completes
    setTimeout(() => setOpeningClaimId(null), 200)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setIsAdvanceDialogOpen(true)} disabled={isLoading}>
        ...
        <TabsContent value="approvals">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Review and process employee reimbursement claims.</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
        ...
        <TabsContent value="all">
          <Card>
            <CardHeader>
              ...
              <CardDescription>A complete log of all claims and advances. Click a row to view details.</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                          ...
                          {claim.receipt_url && (
                            <Button variant="outline" size="sm" asChild onClick={(e) => e.stopPropagation()}>
                              <a href={claim.receipt_url} target="_blank" rel="noopener noreferrer">
                                {openingClaimId === `receipt-${claim.id}` ? (
                                  <>
      ...
      <AdminExpenseDetailsDialog
        ...
        onActionSuccess={fetchData}
      />
      <AdvanceDialog open={isAdvanceDialogOpen} onOpenChange={setIsAdvanceDialogOpen} onSuccess={fetchData} />
    </div>
  )
}
