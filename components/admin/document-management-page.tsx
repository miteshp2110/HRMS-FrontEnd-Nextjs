...
import { AttendanceAuditLog, getExpiringDocuments, getDocumentTypes, getAllUserProfiles, getEmployeeDocuments, uploadEmployeeDocument, deleteUploadedDocument, searchUsers, type DocumentType, type UserProfile, type EmployeeDocument } from "@/lib/api";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
...
const UploadDocumentDialog = ({
  ...
  const [formData, setFormData] = useState({ employeeId: "", document_id: "", expiry_date: "" });
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!selectedFile || !formData.document_id || !formData.employeeId) {
      toast({
        ...
        description: "Please select an employee, a document type, and a file.",
        variant: "destructive",
      });
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select an employee, document type, and file to upload.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleUpload} className="space-y-4 py-4">
          <div className="space-y-2">
          ...
          <Select
            onValueChange={(val) =>
              setFormData((f) => ({ ...f, document_id: val }))
            }
          >
            <SelectTrigger>
...
export function DocumentManagementPage() {
  ...
  const [userDocuments, setUserDocuments] = useState<Map<number, EmployeeDocument[]>>(new Map());
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const allDocsPromises = allUsers.map((user) =>
        getEmployeeDocuments(user.id).then((docs) => ({
          userId: user.id,
          docs,
        }))
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Upcoming Expiries (Next 30 Days)</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="space-y-1">
              {[...Array(3)].map((_, idx) => (
                <div className="flex gap-6 items-center mb-2" key={idx}>
            ...
            <p className="text-muted-foreground">No documents are expiring soon.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {expiringDocs.map((doc) => (
                  <TableRow key={doc.id}>
                    ...
                    <TableCell className="text-red-500 font-medium">
                      {new Date(doc.expiry_date).toLocaleDateString("en-GB").replace(/\//g, "-")}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Document Explorer</CardTitle>
          <CardDescription>
            Filter by document type to see active employee compliance.
          </CardDescription>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
            <Select value={selectedDocType} onValueChange={setSelectedDocType}>
              <SelectTrigger>
                <SelectValue placeholder="Select a document type to begin..." />
              </SelectTrigger>
              <SelectContent>
                {docTypes.map((t) => (
                  <SelectItem key={t.id} value={String(t.id)}>
        ...
        <CardContent>
          {!selectedDocType ? (
            <div className="text-center py-12 text-muted-foreground">
              Please select a document type to see compliance data.
            </div>
          ) : isLoading ? (
            <div className="space-y-2">
              {[...Array(8)].map((_, idx) => (
                ...
                <TableBody>
                  {filteredUsers.map((user) => {
                    ...
                    return (
                      <TableRow key={user.id} className={!doc ? "bg-red-500/10" : ""}>
                        <TableCell>
                          <Link href={`/directory/${user.id}`} className="font-medium text-primary hover:underline">
                            {user.first_name} {user.last_name}
                          </Link>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {doc ? new Date(doc.upload_date).toLocaleDateString("en-GB").replace(/\//g, "-") : "N/A"}
                        </TableCell>
                        <TableCell>
                          {doc?.expiry_date
                            ? new Date(doc.expiry_date).toLocaleDateString("en-GB").replace(/\//g, "-")
                          ...
                          {doc && (
                            <>
                              <Button variant="ghost" size="sm" asChild>
                                <a href={doc.upload_link} target="_blank" rel="noopener noreferrer">
