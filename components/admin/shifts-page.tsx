...
export function ShiftsPage() {
  ...
  // Format time to HH:MM only (24-hour format)
  ...
  const handleCreateShift = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Get time values and ensure HH:MM format only (24-hour)
    const fromTime = formatTimeToHHMM(formData.get("from_time_local") as string);
    const toTime = formatTimeToHHMM(formData.get("to_time_local") as string);
    
    const data = {
      name: formData.get("name") as string,
      from_time_local: fromTime,
      to_time_local: toTime,
      timezone: selectedTimezone ?? "UTC",
      half_day_threshold: Number(formData.get("half_day_threshold")),
      punch_in_margin: Number(formData.get("punch_in_margin")),
      punch_out_margin: Number(formData.get("punch_out_margin")),
  ...
  // Format UTC time to timezone and display in 12-hour AM/PM format
  const formatTimeInTimezone = (utcTime: string, timeZone: string): string => {
  ...
  // Convert UTC time to local timezone for input fields (24-hour HH:MM format)
  function formatUtcTimeToZone(timeString: string | undefined, timeZone: string): string {
    if (!timeString) return "";
    ...
    <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
              ...
              <DialogContent>
                ...
                <form ref={createFormRef} onSubmit={handleCreateShift} className="space-y-4 py-4">
                  {/* Timezone Display */}
                  <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
                    <Globe className="h-4 w-4 text-muted-foreground" />
                    <span className="text-sm text-muted-foreground">
                      Timezone: <span className="font-semibold text-foreground">{selectedTimezone}</span>
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="from_time_local">From Time (24h) *</Label>
                      <Input 
                        id="from_time_local" 
                        name="from_time_local" 
                        type="time" 
                    ...
                    <div>
                      <Label htmlFor="to_time_local">To Time (24h) *</Label>
                      <Input 
                        id="to_time_local" 
                        name="to_time_local" 
                        type="time" 
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit Shift: {selectedShift?.name}</DialogTitle>
            <DialogDescription>Update the shift details. All fields will be updated.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEditShift} className="space-y-4 py-4">
            {/* Timezone Display */}
            <div className="flex items-center gap-2 p-3 bg-muted rounded-md">
            ...
            <div>
              ...
              <Input id="edit_name" name="name" defaultValue={selectedShift?.name} required />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit_from_time_local">From Time (24h) *</Label>
                <Input
                  type="time"
                  step="60"
                  id="edit_from_time_local"
                  name="from_time_local"
                  defaultValue={formatUtcTimeToZone(selectedShift?.from_time, selectedTimezone!)}
                  required
                />
              ...
              <div>
                <Label htmlFor="edit_to_time_local">To Time (24h) *</Label>
                <Input
                  type="time"
                  step="60"
                  id="edit_to_time_local"
                  name="to_time_local"
                  defaultValue={formatUtcTimeToZone(selectedShift?.to_time, selectedTimezone!)}
                  required
                />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit_half_day_threshold">Half Day (Hrs) *</Label>
                <Input
                  id="edit_half_day_threshold"
                  name="half_day_threshold"
                  type="number"
                  step="0.5"
                  min="0"
              ...
              <div>
                <Label htmlFor="edit_overtime_threshold">Overtime Threshold (Mins) *</Label>
                <Input
              ...
              <div>
                <Label htmlFor="edit_punch_in_margin">Grace In (Mins) *</Label>
                <Input
                  id="edit_punch_in_margin"
                  name="punch_in_margin"
                  type="number"
                  min="0"
                  defaultValue={selectedShift?.punch_in_margin}
              ...
              <div>
                <Label htmlFor="edit_punch_out_margin">Grace Out (Mins) *</Label>
                <Input
                  id="edit_punch_out_margin"
                  name="punch_out_margin"
                  type="number"
                  min="0"
                  defaultValue={selectedShift?.punch_out_margin}
