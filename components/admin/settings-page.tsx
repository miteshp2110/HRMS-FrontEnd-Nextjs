...
const mainTabsConfig = [
  { value: "general", title: "General", permission: "user.manage" },
  ...
  { value: "cases", title: "Case Types", permission: "cases.manage" },
]

const generalTabsConfig = [
  { value: "timezone", title: "Timezone", permission: "user.manage" },
  { value: "name-series", title: "Name Series", permission: "user.manage" },
...
export function SettingsPage() {
  ...
  // Determine active tabs based on URL params
  const activeMainTab = useMemo(() => {
    if (!tabParam) return visibleMainTabs[0]?.value || "general"
    
    // If section param exists, we're in general tab
    if (sectionParam) return "general"
    
    // Otherwise use the tab param if it's valid
    const isValidTab = visibleMainTabs.some(t => t.value === tabParam)
    return isValidTab ? tabParam : visibleMainTabs[0]?.value || "general"
  }, [tabParam, sectionParam, visibleMainTabs])

  const activeGeneralTab = useMemo(() => {
    if (sectionParam) {
      const isValidSection = visibleGeneralTabs.some(t => t.value === sectionParam)
      return isValidSection ? sectionParam : visibleGeneralTabs[0]?.value || "timezone"
    }
    return visibleGeneralTabs[0]?.value || "timezone"
  }, [sectionParam, visibleGeneralTabs])
      ...
      <Tabs value={activeMainTab} onValueChange={handleMainTabChange} className="w-full">
        ...
        {hasPermission("user.manage") && (
          <TabsContent value="general" className="mt-6">
            <Tabs value={activeGeneralTab} onValueChange={handleGeneralTabChange}>
