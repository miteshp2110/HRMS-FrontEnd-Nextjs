...
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Plus, Edit, Trash2, Briefcase, AlertCircle, Clock } from "lucide-react";
import { getJobs, createJob, updateJob, deleteJob, type Job } from "@/lib/api";
import { useToast } from "@/hooks/use-toast";
import { JobEmployeesDialog } from "./job-employees-dialog";
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Job Titles</CardTitle>
          <CardDescription>
            Manage available job titles in the organization. Click a title to see assigned employees.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
                ...
                <TableRow key={job.id}>
                  <TableCell className="font-medium">
                    <span
                      ...
                      onClick={() => handleOpenEmployeesDialog(job)}
                    >
                      <Briefcase className="h-4 w-4 text-muted-foreground" />
                      {job.title}
                    </span>
          ...
          <form onSubmit={handleEditJob}>
            <div className="space-y-4 py-4">
              <div>
                ...
                <Input id="edit-title" name="title" defaultValue={selectedJob?.title} required />
              </div>
              <div>
      ...
      <JobEmployeesDialog job={selectedJob} open={employeesDialogOpen} onOpenChange={setEmployeesDialogOpen} />
    </div>
  );
}
