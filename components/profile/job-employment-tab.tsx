...
// Skeleton component for employment details
function EmploymentDetailsSkeleton() {
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
...
export function JobEmploymentTab({ 
  profile, 
  isEditing, 
  isSelfProfile = false, 
  onSave, 
  ...
  const handleDeactivate = async () => {
    if(directReports.length > 0) {
      toast({ 
        title: "Error", 
        description: "There are employees reporting to this user. Please make changes before deactivating.", 
        variant: "destructive" 
      })
      return
    }
    ...
    try {
      ...
      toast({ 
        title: "Success", 
        description: `${profile.first_name} has been deactivated.` 
      })
      setIsDeactivateDialogOpen(false)
      await onSave({inactive_reason:"@@##@@"}) // Trigger a profile refresh
    } catch(error: any) {
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      toast({ 
        title: "Success", 
        description: `${profile.first_name} has been activated.` 
      })
      await onSave({inactive_reason:"@@##@@"}) // Trigger a profile refresh
    } catch (error: any) {
      toast({ 
        title: "Error", 
        description: `Failed to activate: ${error.message}`, 
        variant: "destructive" 
      })
    } finally {
  ...
  if (isLoading) {
    return <EmploymentDetailsSkeleton />
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Select 
                  ...
                  onValueChange={(value) => handleSelectChange("job_role", value)}
                  disabled={isSaving}
                >
                  <SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Select 
                  ...
                  onValueChange={(value) => handleSelectChange("system_role", value)}
                  disabled={isSaving}
                >
                  <SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Popover open={isManagerPopoverOpen} onOpenChange={setIsManagerPopoverOpen}>
                  <PopoverTrigger asChild>
                    <button 
                      className="w-full flex items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground shadow-sm hover:bg-accent hover:text-accent-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                      disabled={isSaving}
                    >
                      {managers.find(m => m.id === formData.reports_to)?.first_name || "Select Manager"}
                    </button>
                  ...
                  <PopoverContent className="w-[300px] p-0">
                    <Command>
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {managers.filter(m => 
                            m.first_name.toLowerCase().includes(managerSearch.toLowerCase())
                          ).map(m => (
                            <CommandItem 
                              key={m.id} 
                              onSelect={() => { 
                                handleSelectChange("reports_to", String(m.id))
            ...
            <div className="space-y-2 flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {profile.is_active ? "Currently active" : "Currently inactive"}
                </p>
              </div>
              {!isSelfProfile && (
                profile.is_active ? (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="inline-block">
                          <Button 
                            ...
                            disabled={directReports.length > 0 || isSaving}
                          >
                            Deactivate User
                          </Button>
                        </div>
                      ...
                      {directReports.length > 0 && (
                        <TooltipContent>
                          <p>This user has direct reports and cannot be deactivated.</p>
                        </TooltipContent>
                      )}
                    </Tooltip>
                  </TooltipProvider>
            ...
            <div className="space-y-2 flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
              ...
              {isEditing ? (
                <Select 
                  name="is_probation" 
                  value={String(!!formData.is_probation)} 
                  onValueChange={(value) => setFormData({...formData, is_probation: value === 'true'})}
                  disabled={isSaving}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="false">Probation Period Over</SelectItem>
                  </SelectContent>
                </Select>
              ) : (
                <Badge 
                  variant={profile.is_probation ? "secondary" : "default"} 
                  className={profile.is_probation ? "bg-orange-100 text-orange-800" : "bg-green-100 text-green-800"}
                >
                  {getProbationStatus()}
                </Badge>
              )}
          ...
          {!profile.is_active && !isEditing && (
            <div className="mt-4 p-4 bg-muted rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <Link href={`/directory/${profile.inactivated_by}`}>
                  {profile.inactivated_by_name}
                </Link>
              </p>
            </div>
          )}
      ...
      {!isEditing && (
        <Card>
          <CardHeader>
            <CardTitle>Direct Reports</CardTitle>
            <CardDescription>
              Employees who report to {profile.first_name}.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingReports ? (
              ...
              <p className="text-muted-foreground text-sm">
                This user does not manage any employees.
              </p>
            ) : (
              <div className="space-y-4">
                {directReports.map(report => (
                  <div 
                    key={report.id} 
                    className="flex items-center space-x-4 p-2 rounded-md hover:bg-muted/50"
                  >
                    <Avatar>
                      <AvatarImage className="object-cover" src={report.profile_url} />
                      <AvatarFallback>
